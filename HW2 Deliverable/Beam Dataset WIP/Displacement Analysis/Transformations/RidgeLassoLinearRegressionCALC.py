...
ridge_model = Ridge()
ridge_grid_search = GridSearchCV(ridge_model, param_grid, cv=5, scoring='neg_mean_squared_error')  # 5-fold cross-validation
ridge_grid_search.fit(X_train, y_train)

# Lasso Regression with GridSearchCV
lasso_model = Lasso()
lasso_grid_search = GridSearchCV(lasso_model, param_grid, cv=5, scoring='neg_mean_squared_error')  # 5-fold cross-validation
...
print(f"{'Actual Displ (Test)':<25}{'Ridge Predicted Displ (Test)':<30}{'Lasso Predicted Displ (Test)':<30}")
print("-" * 85)
...
print(f"{'Actual Displ (Train)':<25}{'Ridge Predicted Displ (Train)':<30}{'Lasso Predicted Displ (Train)':<30}")
print("-" * 85)
...
sns.scatterplot(x=y_test, y=ridge_pred_test, color='blue', label='Ridge Predicted Displ (Test)', alpha=0.7)

# Plot Actual Stress vs Lasso Predicted Stress (Test Data)
sns.scatterplot(x=y_test, y=lasso_pred_test, color='red', label='Lasso Predicted Displ (Test)', alpha=0.7)
...
# Assuming the necessary libraries and model (ridge_best_model or lasso_best_model) have been defined earlier

# Define the new data point (for displacement = 2mm)
new_data = pd.DataFrame({
    'Volume Fraction': [0.4],  # Example value, adjust as needed
    'Loads': [3],       # Set displacement to 2mm
...
print(f"Predicted Displacement (Ridge Regression): {ridge_prediction[0]}")
print(f"Predicted Displacement (Lasso Regression): {lasso_prediction[0]}")
