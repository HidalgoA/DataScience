...
X = df[['Volume Fraction', 'Displacement', 'Youngs_Modulus', 'Poisson_Ratio']]
X += (np.abs(X.min()) + 1e-6)  # Shift values to be positive
...
y = df['Stress_Values']
y += (np.abs(y.min()) + 1e-6)  # Shift values to be positive
y_transformer = PowerTransformer(method='box-cox', standardize=True)
y = y_transformer.fit_transform(y.values.reshape(-1, 1)).flatten()
