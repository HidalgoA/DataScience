...
def flatten_and_log_transform_csv(csv_path, target_length=241):
    ...
    shifts the data so all values become positive (if needed),
    and flattens the result into a 1D array of length 2*target_length.
    
    Note: Log transformation has been removed.
    """
...
# ---------------------------------------------------------
# 4. Process Each CSV File: Flatten, Resample, and Transform
# ---------------------------------------------------------
raw_features_list = []
category_list = []
metal_type_list = []
...
df_features = pd.concat([
    ...
    df_raw.reset_index(drop=True),          # transformed raw CSV data (482 columns)
...
# ---------------------------------------------------------
# Here we apply no further transformation to the already transformed raw CSV data and material properties.
