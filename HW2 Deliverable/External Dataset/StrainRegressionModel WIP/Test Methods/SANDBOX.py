...
# Load the dataset
file_path = r"C:\Users\hidal\Desktop\Classes\GRAD\Data Sci Mech of Mat\HW2 Deliverable\External Dataset\StrainRegressionModel\Processed_Material_Data_All_Loads.xlsx"
...
# Exclude Stress and Strain from normalization
mechanical_properties = X.drop(columns=['Stress', 'Strain'])  # Only the mechanical properties

# Normalize the mechanical properties using StandardScaler
scaler = StandardScaler()
mechanical_properties_scaled = scaler.fit_transform(mechanical_properties)  # Normalize only the mechanical properties

# Replace the original mechanical properties with the scaled ones
X_scaled = pd.DataFrame(mechanical_properties_scaled, columns=mechanical_properties.columns)
X_scaled[['Stress', 'Strain']] = X[['Stress', 'Strain']]  # Keep Stress and Strain as is
...
# Prepare the data for correlation analysis (normalized mechanical properties + Stress and Strain)
X_scaled[['Fatigue_Life']] = y  # Add the dependent variable for correlation
corr_matrix = X_scaled.corr()  # Calculate the correlation matrix

# Plot the heatmap
plt.figure(figsize=(10, 8))
sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', linewidths=0.5, fmt='.2f', cbar=True)
plt.title('Correlation Heatmap of Normalized Mechanical Properties and Fatigue Life')
plt.show()
