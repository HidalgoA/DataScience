...
for name, model in models.items():
    ...
    y_pred_original = np.exp(y_pred_log) - shift_constant  # Reverse the log transformation
    ...
    results.append([name, model.alpha if hasattr(model, "alpha") else "N/A", r2_original, mae_original, mse_original])
    
# Visualization
plt.figure(figsize=(8, 6))

# Scatter plot for Ridge
sns.scatterplot(x=y_test, y=model_predictions.get("Ridge Regression", y_pred_original), color='blue', label='Ridge Prediction', alpha=0.6)

# Scatter plot for Lasso
sns.scatterplot(x=y_test, y=model_predictions.get("Lasso Regression", y_pred_original), color='red', label='Lasso Prediction', alpha=0.6)
