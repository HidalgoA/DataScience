...
# ---------------------------------------------------------
# 1. Configuration and Paths (for stress-controlled dataset)
# ---------------------------------------------------------
...
TARGET_LENGTH = 241  # Fixed length for resampling CSV data
...
def detect_path_type(col1, col2):
    ...
    Classifies the loading path:
      ...
      - 'Proportional' if the ratio col2/col1 is nearly constant.
...
def resample_to_fixed_length(x, target_length):
    """Resamples a 1D array x to target_length points using linear interpolation."""
...
def flatten_raw_csv(csv_path, target_length=241):
    """
    Reads a CSV file (assumed 2 columns), resamples each column to target_length,
    and flattens them into a single 1D array of length 2*target_length.
    """
    ...
    return np.concatenate([col1_resampled, col2_resampled])

def flatten_yeojohnson_transform_csv(csv_path, target_length=241):
    """
    Reads a CSV file (assumed 2 columns), resamples each column to target_length,
    flattens them into a single 1D array (length = 2*target_length), and applies a
    Yeo–Johnson transformation using scikit-learn's PowerTransformer.
    Yeo–Johnson can handle zero and negative values.
    """
    raw = flatten_raw_csv(csv_path, target_length)
    pt = PowerTransformer(method='yeo-johnson')
    transformed = pt.fit_transform(raw.reshape(-1, 1)).ravel()
    return transformed

# For this version, we use Yeo–Johnson only (no MinMax scaling)
flatten_function = flatten_yeojohnson_transform_csv
...
# Convert material property columns to numeric (cleaning stray spaces)
for col in material_cols:
    df_master[col] = pd.to_numeric(df_master[col].astype(str).str.replace(" ", ""), errors='coerce')

# ---------------------------------------------------------
# 4. Process Each CSV File: Flatten, Resample, and Yeo–Johnson Transform
# ---------------------------------------------------------
raw_features_list = []
category_list = []
metal_type_list = []
...
for csv_file in df_master['load']:
    ...
    try:
        raw_vec = flatten_function(full_path, TARGET_LENGTH)
        half = len(raw_vec) // 2
        col1 = raw_vec[:half]
        col2 = raw_vec[half:]
...
df_features = pd.concat([
    ...
    df_raw.reset_index(drop=True),          # Yeo–Johnson transformed raw CSV data
...
# ---------------------------------------------------------
# Pass through numerical features and one-hot encode categorical features.
...
# ---------------------------------------------------------
# 9. Build Pipelines for Ridge, Lasso, and ElasticNet Regression (without Polynomial Features)
# ---------------------------------------------------------
