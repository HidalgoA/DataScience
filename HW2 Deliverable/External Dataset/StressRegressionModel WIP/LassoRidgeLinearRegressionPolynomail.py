...
# Data processing pipeline for material properties (without PolynomialFeatures)
# Identify columns to exclude from scaling (CSV columns and 'Nf(label)' column)
exclude_cols = ['load', 'Nf(label)']
num_cols = pd_train.select_dtypes(exclude=['object']).columns.tolist()
num_cols = [col for col in num_cols if col not in exclude_cols]  # Exclude CSV and 'Fatigue Life' columns

# Define pipeline for numerical columns (no polynomial features for material data)
num_pipe = Pipeline([
    ('imputer', SimpleImputer(strategy='median')),
    ('scaler', StandardScaler())  # Only scale selected columns
])
...
# Transform the material properties data
x_all_material = ct.fit_transform(pd_train)
...
x_all_material = x_all_material[valid_y_indices]
csv_value_array = csv_value_array[valid_y_indices]
y_all = y_all[valid_y_indices]

# Apply PolynomialFeatures to the CSV data
poly = PolynomialFeatures(degree=2)  # You can adjust degree here
csv_value_poly = poly.fit_transform(csv_value_array)

# Combine material data and CSV polynomial data
combined_data = np.hstack((csv_value_poly, x_all_material))
