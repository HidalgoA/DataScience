...
from sklearn.model_selection import RepeatedKFold, LeaveOneOut, KFold
...
for target in targets:
    ...
    cv = RepeatedKFold(n_splits=3, n_repeats=5, random_state= 150)
    #cv = LeaveOneOut()
    ...
    ridge = RidgeCV(alphas=alphas, cv=cv, scoring='neg_mean_squared_error').fit(X_train, Y_train) #Ridge Regression
    lasso = LassoCV(alphas=alphas, cv=cv, random_state=200).fit(X_train, Y_train) #Lasso Regression
    ...
    print(f"  Ridge  - Best Alpha: {ridge.alpha_:.4f}, R²: {r2_ridge:.3f}, MSE: {mse_ridge:.3f}")
    print(f"  Lasso  - Best Alpha: {lasso.alpha_:.4f}, R²: {r2_lasso:.3f}, MSE: {mse_lasso:.3f}")
    ...
    plt.plot(X, ridge.predict(X), color="red", label=f"Ridge (α={ridge.alpha_:.4f})")
    plt.plot(X, lasso.predict(X), color="green", label=f"Lasso (α={lasso.alpha_:.4f})")
