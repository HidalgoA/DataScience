...
# **Apply Log Transformation to Independent Variables (Skip Non-Numeric Features)**
X = df.drop(columns=["Displacement_Values"])  # Features (everything except displacement)
X = np.log(X + 1)  # Apply log transformation

y = df["Displacement_Values"]  # Response variable (displacement)

# Compute correlation matrix (after transformation)
...
scaler = RobustScaler()  # Change to MinMaxScaler() if needed
