...
# Define file path for the transformed displacement dataset
...
# Convert Displacement_Values to numeric
df["Displacement_Values"] = pd.to_numeric(df["Displacement_Values"], errors="coerce")

# Drop NaN rows (if any were converted)
df = df.dropna(subset=["Displacement_Values"])

# Define the independent variables (features) and response variable (displacement)
X = df.drop(columns=["Displacement_Values"])  # Features (everything except displacement)
y = df["Displacement_Values"]  # Response variable (displacement)

# Compute correlation matrix (after feature selection)
corr_matrix = X.corr(numeric_only=True)

# Generate heatmap with all numeric features
plt.figure(figsize=(10, 8))
sns.heatmap(corr_matrix, annot=True, cmap="coolwarm", fmt=".2f", linewidths=0.5)
plt.title("Feature Correlation Heatmap (MinMax Scaled Features)")
plt.show()

# Apply MinMax Scaling to independent variables
scaler = RobustScaler()  
