...
# ---- PCA on radial profiles grouped by load ----
...
def compute_and_plot_pca_radial(profiles, labels, target_length=100):
    ...
    # Group scores by load (using loads list: "100N", "500N")
    scores_by_load = {}
    for score, label in zip(scores, labels):
        ...
        for ld in loads:
            if ld in label:
                load_found = ld
                break
        if load_found is None:
            load_found = "Unknown"
        scores_by_load.setdefault(load_found, []).append(score)
    
    window = tk.Toplevel()
    window.title("PCA of Radial Profiles (2 Components) - Grouped by Load")
    fig, ax = plt.subplots(figsize=(8, 6))
    
    # Plot for each load group with color coding
    for ld, group_scores in scores_by_load.items():
        group_scores = np.array(group_scores)
        color = "red" if ld == "100N" else "blue" if ld == "500N" else "black"
        ax.scatter(group_scores[:, 0], group_scores[:, 1], color=color, label=ld)
...
def compute_and_plot_pca_radial_3d(profiles, labels, target_length=100):
    ...
    window = tk.Toplevel()
    window.title("PCA of Radial Profiles (3 Components) - Grouped by Load")
...
# ----PCA eigenimages on entire correlation maps ----
...
# Single-figure approach for correlation maps
